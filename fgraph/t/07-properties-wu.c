...
int main() {
    ...
    tap_ok(oval == FGRAPH_OWEIGHTED, "options ok");
    ...
    tap_ok(ulval == 24, "edge count is 24");
    ...
    tap_ok(res == FGRAPH_SUCCESS, "got count ok");
    tap_ok(ulval == 3, "edge count is 3");
    ...
    tap_ok(res == FGRAPH_SUCCESS, "no such edge");
    tap_ok(wval == 4, "weight is 4");
    ...
    res = fgraph_edge_set_weight(&graph, 6, 4, 20);
    tap_ok(res == FGRAPH_SUCCESS, "set weight ok");
    
    res = fgraph_edge_get_weight(&graph, 4, 6, &wval);
    tap_ok(res == FGRAPH_SUCCESS, "got weight ok");
    tap_ok(wval == 20, "weight is 20");
    ...
    res = fgraph_edge_get_weight(&graph, 4, 2, &wval);
    tap_ok(res == FGRAPH_ENOEDGE, "no such edge");
